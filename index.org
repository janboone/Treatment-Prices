...
Figure [[fig:profitfunctions]] is generated for an Hotelling model of insurer competition. The figure is based on the following parameter values: $\phi=0.5,v_1(\psi_{1h}-\psi_{1l})=1,\psi_2v_2=5,t_h=1,t_l=3,\eta =1,y_l=y_h=0$. Here we give the details of the model and the python code for the figure.
...
\end{align}
where $\lambda_l$ denotes the Lagrange multiplier on $IC_l$ constraint eqref:eq:4. In the main text we use a shortcut and differentiate profits with respect to $x_{1h}$. Strictly speaking, $x_{1h}$ is an endogenous variable chosen by the insurer. Hence, we introduce $\alpha$ as the probability that research lab 1 offers treatment 1 at all to the insurers. 
